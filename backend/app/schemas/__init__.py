...
class NoteBase(BaseModel):
    ...
    folder_id: Optional[int] = Field(
        None, description="Folder ID to organize this note"
    )
...
class NoteUpdate(BaseModel):
    ...
    folder_id: Optional[int] = Field(
        None, description="Move note to a different folder"
    )
...
class NoteResponse(NoteBase):
    ...
    user_id: int
    folder_id: Optional[int] = Field(
        None, description="Folder ID if note is in a folder"
    )
    created_at: datetime
    updated_at: datetime
    tags: list[str] = Field(
        default_factory=list, description="List of tag names associated with this note"
    )
    ...
    def from_orm_with_tags(cls, db_note):
        ...
        tag_names = (
            [tag.name for tag in db_note.tags]
            if hasattr(db_note, "tags") and db_note.tags
            else []
        )
        note_dict = {
            "id": db_note.id,
...
class SearchRequest(BaseModel):
    ...
    tag_mode: TagFilterMode = Field(
        TagFilterMode.AND, description="Tag filter mode: and, or, exclude"
    )
    ...
    updated_after: Optional[datetime] = Field(
        None, description="Filter notes updated after this date"
    )
    updated_before: Optional[datetime] = Field(
        None, description="Filter notes updated before this date"
    )

    # Content filters
    title_only: bool = Field(False, description="Search only in note titles")
    has_attachments: Optional[bool] = Field(
        None, description="Filter notes with/without attachments"
    )
...
class SearchResultItem(BaseModel):
    ...
    match_locations: list[str] = Field(
        ...
        description="Where matches were found: title, content, tags",
    )
...
class FolderBase(BaseModel):
    ...
    parent_id: Optional[int] = Field(
        None, description="Parent folder ID for hierarchical organization"
    )
...
class FolderResponse(FolderBase):
    ...
    note_count: Optional[int] = Field(
        default=0, description="Number of notes in this folder"
    )
...
class SavedSearchCreate(BaseModel):
    ...
    search_query: SearchRequest = Field(
...
class SavedSearchResponse(BaseModel):
    ...
    use_count: int = Field(
        default=0, description="Number of times this search was executed"
    )
...
class SearchSuggestionResponse(BaseModel):
    ...
    relevance_score: float = Field(
        description="Relevance score based on recency and frequency"
    )
