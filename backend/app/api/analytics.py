...
from app.models import User, SearchAnalytics
...
async def get_popular_searches(
    ...
    """
    popular = db.query(SearchAnalytics).filter(
        SearchAnalytics.user_id == current_user.id
    ).order_by(
        desc(SearchAnalytics.search_count),
        desc(SearchAnalytics.last_searched_at)
    ).limit(limit).all()
...
async def get_search_suggestions(
    prefix: str = Query(..., min_length=1, max_length=100, description="Search query prefix"),
    ...
    suggestions = db.query(SearchAnalytics).filter(
        SearchAnalytics.user_id == current_user.id,
...
async def get_trending_searches(
    limit: int = Query(10, ge=1, le=50, description="Number of trending searches to return"),
    days: int = Query(7, ge=1, le=30, description="Number of days to analyze for trends"),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """
    ...
    all_analytics = db.query(SearchAnalytics).filter(
        SearchAnalytics.user_id == current_user.id,
    ...
    for item in all_analytics:
        ...
        if item.last_searched_at.replace(tzinfo=None) >= cutoff_date:
            ...
            recent_ratio = min(1.0, (datetime.utcnow() - item.last_searched_at.replace(tzinfo=None)).days / days)
...
async def get_search_stats(
    ...
    """
    # Total searches
    total_searches = db.query(func.sum(SearchAnalytics.search_count)).filter(
        SearchAnalytics.user_id == current_user.id
    ).scalar() or 0
    
    # Unique queries
    unique_queries = db.query(func.count(SearchAnalytics.id)).filter(
        SearchAnalytics.user_id == current_user.id
    ).scalar() or 0
    
    # Average results per search
    avg_results = db.query(func.avg(SearchAnalytics.avg_result_count)).filter(
        SearchAnalytics.user_id == current_user.id,
        SearchAnalytics.avg_result_count.isnot(None)
    ).scalar()
    
    # Most searched query
    most_searched = db.query(SearchAnalytics).filter(
        SearchAnalytics.user_id == current_user.id
    ).order_by(desc(SearchAnalytics.search_count)).first()
    
    # Searches today
    today_start = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
    searches_today = db.query(func.sum(SearchAnalytics.search_count)).filter(
        SearchAnalytics.user_id == current_user.id,
    ...
    searches_this_week = db.query(func.sum(SearchAnalytics.search_count)).filter(
        SearchAnalytics.user_id == current_user.id,
