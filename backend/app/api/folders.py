...
from app.schemas import FolderCreate, FolderListResponse, FolderResponse, FolderUpdate
...
async def get_folders(
    ...
    for folder in folders:
        note_count = (
            ...
            .filter(Note.folder_id == folder.id, Note.user_id == current_user.id)
            .scalar()
        )
...
async def create_folder(
    ...
    # Create folder
    db_folder = Folder(
        name=folder_data.name, parent_id=folder_data.parent_id, user_id=current_user.id
    )
...
async def update_folder(
    ...
    if (
        folder_update.parent_id is not None
        and folder_update.parent_id != folder.parent_id
    ):
        ...
        while current_parent.parent_id is not None:
            ...
            current_parent = (
                db.query(Folder).filter(Folder.id == current_parent.parent_id).first()
            )
...
async def delete_folder(
    ...
    db.query(Note).filter(
        Note.folder_id == folder_id, Note.user_id == current_user.id
